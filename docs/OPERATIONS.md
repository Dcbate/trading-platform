...
| `FraudDetectionLatencyHigh` | fraud check p99 > 2s for 2m | investigate | Almost always the Gemini API being slow, not the app — see the incident example below |
...
This is almost always the real Gemini API (`GeminiAnomalyDetector`), not the application — check
the `Fraud detection latency` Grafana panel and the `fraud_detection_latency_seconds` metric first.
I built `GeminiAnomalyDetector` to degrade gracefully (falls back to the plain rule-based decision
on any failure or timeout, never blocking the fraud verdict itself — see `DESIGN_DECISIONS.md`), so
a slow or down Gemini API shows up as elevated latency, **not** as failed payments. If it's
consistently slow: nothing urgent needs fixing in the app; consider lowering the enrichment timeout
(currently 2000ms, `GeminiConfig`) if the tail is unacceptable, or removing the `GEMINI_API_KEY` env
var temporarily to force the fast fallback path while Gemini recovers.
...
- **Level 3 (external dependency):** Gemini or Anthropic API outage lasting beyond what the
