...
class PriceControllerTest {
    ...
    void setUp() {
        TradingProperties tradingProperties = new TradingProperties(
                List.of("EUR/USD", "GBP/USD", "USD/JPY"), new TradingProperties.PriceFeed(2000, new BigDecimal("10")));
        mockMvc = MockMvcBuilders.standaloneSetup(new PriceController(priceFeedService, tradingProperties)).build();
    }

    @Test
    void listPricesReturnsOnlyPairsWithACachedPrice() throws Exception {
        ...
        mockMvc.perform(get("/v1/fx/prices"))
                ...
                .andExpect(jsonPath("$[0].currencyPair").value("EUR/USD"))
